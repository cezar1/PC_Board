...
module small_wall()
{
    intersection()
    {
        union()
        {
            ...
            cube([across_width+thickness2,local_thickness,local_height]);
        }
        union()
        {
            cube([local_thickness,local_thickness,local_height]);
            translate([0,local_thickness/2-thickness2/2,0]){
            cube ([across_width,thickness2,thickness2]); 
            translate([0,0,local_height-thickness2]) cube ([across_width,thickness2,thickness2]);
            translate([across_width/2+local_thickness/2,thickness2/2,local_height/2]) rotate([0,local_angle-8,0]) cube([thickness2,thickness2,local_diagonal],center=true);
    ...
    difference()
    {
        union()
        {
            ...
            translate([thickness,0,0]) rotate([0,0,90]) {
                small_wall(local_thickness=local_thickness,local_height=local_height,across_width=across_width,thickness2=thickness2,local_angle=local_angle,local_diagonal=local_diagonal);   
            }
        }
        union()
        {
            translate([thickness/2,thickness/2,0])hex_hole_exit(h_trap=NUT_HEIGHT_M3,h_hole=NUT_HEIGHT_M3*2,r_trap=SCREW_STANDARD_M3,rot=0,l_exit=thickness*2,rot_exit=-180);
        }
    }
